...
final class API implements API_Interface {
	...
	public function get_blacklist() {
		...
		if ( isset( $body['data'] ) ) {
			...
			Plugin::set_persistent_site_transient( 'gd_system_blacklist', $body['data'], 300 );
	...
	public function user_changed_domain( $cname = '' ) {
		...
		if ( false !== ( $transient = get_site_transient( 'gd_system_domain_changed' ) ) ) {
		...
		set_site_transient( 'gd_system_domain_changed', $changed, absint( $timeout ) );
